...
mkdir -p /var/www/$nginx_url/report/
mkdir -p /var/www/$nginx_url/html/
mkdir -p /var/www/$nginx_url/report/
home_page=/var/www/$nginx_url/html

cp ./tms-dp/infra/nginx/nginx.conf /etc/nginx/
...
sed -i "s|REPORT_EXAMPLE|https://$nginx_url/report/|g" $home_page/index.html
...
cp ./tms-dp/infra/nginx/.htpasswd $ND
...
sudo sed -i -e "s/EXAMPLE_PATH/$nginx_url/g" ./tms-dp/infra/ansible/deploy.yaml
echo $prod_url > ./tms-dp/infra/jenkins/prod_url
echo $dev_url > ./tms-dp/infra/jenkins/dev_url
